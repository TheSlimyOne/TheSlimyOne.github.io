...
// AJAX call to retrieve covidData from:
// https://api.covid19api.com/summary
xhttp = new XMLHttpRequest();

now = dayjs()
    //
  if (localStorage.getItem('covidData') == null || dayjs(JSON.parse(localStorage.getItem("lastRequest"))) <= now){
...
function loadContent() {
  ...
  chartData.options.title.text = "Covid 19 Hotspots as of " + dayjs().format("MMMM DD, YYYY");
...
function makeNewArray(){
  ...
  for (let i=0; i<covidJsObj.Countries.length; i++) {
    // Including only the countries that is in population object
